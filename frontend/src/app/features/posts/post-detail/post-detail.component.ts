...
export class PostDetailComponent implements OnInit {
  ...
  reportPost(): void {
    ...
    const dialogRef = this.dialog.open(ReportDialogComponent, {
      ...
      data: {
        type: 'POST', // ← ADD THIS
        targetId: post.id, // ← CHANGE FROM postId
        targetTitle: post.title, // ← CHANGE FROM postTitle
      },
    });

    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        // Use generic submitReport
        this.reportService.submitReport(result.type, result.targetId, result.reason).subscribe({
          next: () => {
            this.snackBar.open('Report submitted successfully.', 'Close', { duration: 5000 });
          },
          error: (error) => {
            const message = error.error?.message || 'Failed to submit report.';
